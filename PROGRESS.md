...
| `agenda` | Customer reports (Day 1, 16:30), Cross-pod show & tell (Day 2, 12:00), Closing & Group Photo (Day 2, 17:00) | Glue deck (intros, framing, closing), reflection worksheet, 90-day rollout template, collect guest decks | Stefan (customer reports), rest TBD | in progress (TS deck started in `modules/agenda`: exercise-overview built, cover + 3 pod-block timeline slides for Day 1 AM, Day 1 PM, Day 2 AM, timings from `dev-bootcamp-factory-workshop`. Glue/framing slides, reflection worksheet, 90-day rollout, guest decks still todo) |
| `reflect-commit` | Reflect & Commit (Day 2, 16:15) | Closing arc deck (10 slides): two-day recap, bottom-up factory roadmap with growing mermaid state machine (phases 1-3 + full blueprint), silent commit questions | Malte | done (TS deck, `modules/reflect-commit`; mermaid state diagrams pre-cached at module load; diagram sourced from `software-factory-blueprint.html`) |
